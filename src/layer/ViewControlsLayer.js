...
define([
    ...
    function (Angle,
        ...
        ViewControlsLayer.prototype.doRender = function (dc) {
            ...
            var selfthis=this;
            var changtime= $("#testchange").val();
           // console.log(changtime);
            name(changtime);
            function name(params) {
                
                if (params != rechangtime) {
                   rechangtime = params;                    
                    var dx = 0.1,
                    dy = 0,
                    oldLat = selfthis.wwd.navigator.lookAtLocation.latitude,
                    oldLon = selfthis.wwd.navigator.lookAtLocation.longitude,
                    // Scale the increment by a constant and the relative distance of the eye to the surface.
                    scale = selfthis.panIncrement
                        * (selfthis.wwd.navigator.range / selfthis.wwd.globe.radiusAt(oldLat, oldLon)),
                    heading = selfthis.wwd.navigator.heading + (Math.atan2(dx, dy) * Angle.RADIANS_TO_DEGREES),
                    distance = scale * Math.sqrt(dx * dx + dy * dy);

                    params = params*10;
            
                    if (params <= 120) { 
                        params = 180 - params*3/2;
                    } else { 
                        params = -(params-120)*3/2;
                    }
                  console.log(oldLon+"===="+params);
                   dx =params - oldLon;
                   if (Math.abs(dx) > 180) {
                       dx=360-dx;
                   }
                   dx = dx/60;
                        ...
                        Location.greatCircleLocation(selfthis.wwd.navigator.lookAtLocation, heading, dx,
                            selfthis.wwd.navigator.lookAtLocation);
                       // console.log(selfthis.wwd.navigator.lookAtLocation);
                     //console.log(index);                                   
                        selfthis.wwd.redraw();   

                    if (dx > 0) {
                      for (let index = 0; index < dx; index+=0.1) {
                    //    Location.greatCircleLocation(selfthis.wwd.navigator.lookAtLocation, heading, -0.1,
                    //         selfthis.wwd.navigator.lookAtLocation);
                    //    // console.log(selfthis.wwd.navigator.lookAtLocation);
                    //  //console.log(index);                                   
                    //     selfthis.wwd.redraw();                         
